...
root_agent = Agent(
    ...
    instruction=(
        ...
        "- Session state (get_preferences/update_preference): owns user identity and "
        "settings (name, customer ID, notification channel, timezone). Always use these "
        "tools for structured preferences.\n"
        "- Memory Bank (automatic): owns conversation history insights — past issues, "
        "ticket outcomes, topics discussed, and explicit instructions from the user. "
        "Do NOT rely on memories for preferences that are already in session state.\n\n"
